...
export function AuthInput({ label, type = 'text', value, onChange, required, minLength, hasToggle, show, onToggle, placeholder, autoComplete }) {
  return (
    <div className="relative bg-[#0a1a2e]/50 lg:bg-white border border-white/15 lg:border-gray-200 rounded-xl px-4 pt-3 pb-2.5 h-[62px] focus-within:border-brand-blue focus-within:ring-2 focus-within:ring-brand-blue/25 lg:focus-within:shadow-[0_0_0_3px_rgba(23,135,254,0.12)] transition-all duration-150">
      <label className="block text-[10px] font-semibold tracking-widest text-white/50 lg:text-gray-400 uppercase mb-0.5">
        {label}
      </label>
      <input
        ...
        className="w-full bg-transparent outline-none text-[14px] text-white lg:text-gray-800 placeholder:text-white/30 lg:placeholder:text-gray-300 pr-8 leading-tight"
      />
      {hasToggle && (
        <button
          type="button"
          onClick={onToggle}
          className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/40 lg:text-gray-400 hover:text-brand-blue transition-colors"
          tabIndex={-1}
