...
import { X, ChevronRight, Check, CheckCircle } from 'lucide-react';
...
/* ─── Per-platform prerequisites (must be acknowledged before connecting) ─── */
...
/* ─── Platform brand colors for the modal Continue button ─── */
...
/* ─── Pre-connect requirements modal ─── */
function ConnectModal({ platformId, platformName, onClose, onConfirm }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[80] flex items-end lg:items-center justify-center p-0 lg:p-4 bg-black/50 backdrop-blur-sm"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="w-full lg:max-w-md bg-white rounded-t-3xl lg:rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col">
        {/* Mobile drag handle */}
...
export default function LinkAccountsSheet({ open, onClose }) {
  ...
  // null | { id, name } — platform awaiting prerequisite acknowledgement
  ...
  return (
    ...
    >
      ...
      {/* Prerequisites modal — shown before any actual connect flow */}
      {connectModal && (
        <ConnectModal
