...
export default function PlatformBadge({ platform, size = 'md', className = '' }) {
  ...
  if (size === 'icon') {
    ...
    return (
      <span className={`${colorMap[platform] || 'text-gray-500'} ${className}`}>
  ...
  if (size === 'sm') {
    return (
      <div className={`w-5 h-5 ${config.color} rounded-full border-2 border-white flex items-center justify-center ${className}`} title={config.label}>
  ...
  if (size === 'xs') {
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${config.bgLight} ${config.textColor}`}>
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold ${config.bgLight} ${config.textColor} ${className}`}>
